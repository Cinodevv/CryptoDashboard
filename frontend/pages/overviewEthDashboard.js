...
const OverviewEthDashboard = () => {
  ...
  const [mendohRinkeby] = useState(
    "0xA6c3E9673B4698D7Ad0596585e4529e6713A8836"
  );
  const [chainlinkAddress] = useState(
  ...
  const getLinkAmtHandler = async () => {
    try {
      if (address != undefined) {
        const chainlinkAmt = await vmChainlinkContract.methods
          .balanceOf(address)
          .call();
        const chainlinkDec = await vmChainlinkContract.methods
          .decimals()
          .call();
            ...
            <section>
              <div className="columns is-9 mt-3 is-centered is-transparent">
                <header className="card-header  is-transparent">
                  <p className="card-header-title is-transparent">
                    Wallet Balances
                  </p>
                </header>
              </div>
